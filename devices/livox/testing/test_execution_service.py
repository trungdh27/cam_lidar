...
class TestExecutionService(QObject):
    ...
    def __init__(self, registry, evidence_store=None, parent=None):
        ...
        self.evidence_store = evidence_store or LidarEvidenceStore()
        self.running = False
    ...
    def start(self, test_ids: list[str], context) -> bool:
        ...
        if not definitions:
            return False
        stream_was_running = context.stream_process_active
        try:
            session_id = self.evidence_store.begin_session(context.device)
            initialize_session = getattr(
                self.evidence_store,
                "initialize_session",
                None,
            )
            if initialize_session is not None:
                ...
                initialize_session(
                    session_id,
                    {
                        ...
                        "detected_model": identity.get("model"),
                        "serial": identity.get("serial"),
                        "lidar_ip": context.lidar_ip,
                        "jetson_host": getattr(
                            context.jetson_state,
                            "host",
                            None,
                        ),
                        "jetson_lidar_interface": (
                            context.network_profile.jetson.interface
                        ),
                        "test_ids": [item.id for item in definitions],
        ...
        self._stream_was_running_before_tests = stream_was_running
        self._runner_started_stream = False
        self._stream_failure = None
    ...
    def _missing_prerequisite(self, definition) -> str | None:
        if (
            getattr(definition, "requires_ros2_target", False)
            and not bool(getattr(self.context, "ros2_target_configured", False))
        ):
            return "Production ROS2 target is not configured"
    ...
    def _finalize_plan(self) -> None:
        ...
        if finalize_session is not None:
            identity = self.context.discovery_result or {}
            try:
                finalize_session(
                    self.session_id,
                    counts=dict(counts),
                    ...
                    duration_sec=max(
                        0.0,
                        time.monotonic()
                        - (self._plan_started_at or time.monotonic()),
                    ),
                    detected_model=identity.get("model"),
                    serial=identity.get("serial"),
                )
                ...
                self._emit_log(
                    "ERROR",
                    "Unable to finalize LiDAR session manifest: "
