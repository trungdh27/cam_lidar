...
class TestContext:
    ...
    ros2_target_profile: Any = None
    lidar_ros2_service: Any = None
    guided_provider: Any = None

    @property
    def ros2_target_configured(self) -> bool:
        profile = self.ros2_target_profile
        return bool(
            profile is not None
            and getattr(profile, "enabled", False)
            and self.lidar_ros2_service is not None
        )
